...
void MainWindow::paintEvent(QPaintEvent *event)
{
    ...
    // 绘制棋盘
    QImage background;
    background.load(":/pic/wooden2.jpg");
    background=background.scaled(kBoardMargin + kBlockSize*(kBoardSizeNum + 1),kBoardMargin + kBlockSize*(kBoardSizeNum + 1));
...
//    painter.setPen(pen);
    ...
    if(game_type==BOT&&game->playerFlag==true){
        brush.setColor(Qt::white);  //绘制边框
        painter.setBrush(brush);
        painter.drawEllipse(kBoardMargin + kBlockSize *game->latestcol- kRadius-2,kBoardMargin + kBlockSize *game->latestrow- kRadius-2, kRadius*2+4,kRadius*2+4);
    }
...
{
    int* rowcol = game->actionByAI();
//    QSound::play(CHESS_ONE_SOUND);
    update();
    if (game->isWin(rowcol[0], rowcol[1]) && game->gameStatus == PLAYING) //加上判黑赢的代码
    {
        qDebug() << "win";
        game->gameStatus = WIN;
        QSound::play(WIN_SOUND);
        QString str;
        if (game->gameMapVec[rowcol[0]][rowcol[1]] == 1)
            str = "white player";
        else if (game->gameMapVec[rowcol[0]][rowcol[1]] == -1)
            str = "black player";
        QMessageBox::StandardButton btnValue = QMessageBox::information(this, "congratulations", str + " win!");
